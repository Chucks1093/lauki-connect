...
export function ConnectButton() {
	...
	const isAuthenticated = Boolean(
		authSession?.authenticated && authSession.address
	);
	...
	if (!isAuthenticated || !authSession?.address) {
		return (
			<button
				className="inline-flex h-10 items-center justify-center rounded-xl bg-neutral-700 px-5 text-[12px] font-semibold uppercase tracking-[0.16em] text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-70"
